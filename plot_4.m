%% 2020-9-4 NSP_ANC 2018_nolinear
...
for i = 1:L
    ...
    d(i) = x_input(1) + 0.8 * x_input(2) + 0.3 * x_input(3) + 0.4 * x_input(4) - 0.8 * x_input(1) * x_input(2) + 0.9 * x_input(1) * x_input(3) + 0.7 * x_input(1) * x_input(4);
end
...
nfft = fs/8; 
E_main = zeros(M,1);
y_main = zeros(nfft/2,1);
d_main = zeros(nfft/2,1);
...
for miu3 =0
    ...
    for k = 1:T
        ...
        for i = 1:M
            ...
            W2(:,1) = W2(:,1) - miu2 * FEn * X2 * ys1(Beita+1)/(Y2(1)^2 + yita);
            W2(:,2) = W2(:,2) - miu2 * FEn * X2 * ys2(Beita+1)/(Y2(1)^2 + yita);

            W3(:,1) = W3(:,1) - miu3 * FEn * X2 * ys21(Beita+1)/(Y3(1)^2 + yita2);
            W3(:,2) = W3(:,2) - miu3 * FEn * X2 * ys22(Beita+1)/(Y3(1)^2 + yita2);
            W3(:,3) = W3(:,3) - miu3 * FEn * X2 * ys23(Beita+1)/(Y3(1)^2 + yita2);
